	...
	if($c!=NULL)
	  {
		...
		foreach($c as $c1)
		  {
		  $sel=mysqli_query($conn,"select * from items where dat='$c1'");
		  ...
		  $t=date("d-m-y h-i-s");
		   ...
		   mysqli_query($conn,"insert into trash values('$catg','$subcatg','$img','$itemno','$price','$desc','$t')");
		   mysqli_query($conn,"delete from items where dat='$c1'");
		   $flag=1;
		   }
		   if($flag==1)
		     {
